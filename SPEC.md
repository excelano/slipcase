...
A Slipcase container is a ZIP archive that binds a single content file to a flyleaf describing it, so that the two travel as one file.
...
**Flyleaf** — the archive member named `slipcase.flyleaf.toml`.  
**Content file** — the archive member named by the `content.file` key.  
...
- exactly one member named `slipcase.flyleaf.toml`, conforming to §2.2; and
- exactly one member whose name equals the value of `content.file`, conforming to §2.3.
...
Names are decoded before they are compared: as UTF-8 where general purpose bit 11 is set, and as CP437 otherwise. Comparison is then exact over the decoded sequence of code points. It is case-sensitive, and no Unicode normalization is applied to either side. An implementation MUST apply the same decoding and the same comparison when matching `content.file` against member names.

### 2.2 The flyleaf

The flyleaf MUST be a valid [TOML 1.1.0](https://toml.io/en/v1.1.0) document, encoded as UTF-8. Where the member is encrypted, this applies to its decrypted bytes.
...
- **`content.file`** — string. The name of the content file, subject to §2.3.
...
This specification defines no canonical TOML serialization, because TOML defines none. Two writers producing the same flyleaf will not necessarily produce identical bytes.

### 2.3 The content file

A container has exactly one content file. It MAY be of any type and any length, including zero.
...
- not equal `slipcase.flyleaf.toml`.

In short, `content.file` is a plain filename and never a path.

Only the names `.` and `..` are excluded, not the sequence `..` wherever it occurs. `a..b` is a permitted name.

The content file MUST be a regular file entry. Directory entries, symbolic links, and every other entry type a ZIP implementation can record are excluded.

A content file MAY itself be a container. This requires no special handling and has no defined meaning.

### 2.4 The version key

`slipcase_version` names the version of this specification, not the version of the content file and not the revision of this document. Editorial revisions — corrections, clarifications, added examples — do not change it. It changes only when what counts as a conformant container changes.

Conformance is relative to a version. This document states whether a container declaring `1.1` conforms to it and states nothing about one declaring any other value. A container declaring `2.0`, or a value naming no version at all, is outside this document's conformance question rather than failing it. This specification constrains the value's type and not its shape.
...
Neither appears inside a container. A container is identified by opening it and finding a conformant flyleaf; this specification reserves no magic bytes and defines no identification by content.

## 5. Out of scope for this version

This version defines no signature or attestation mechanism, no encryption of its own, no aggregation of multiple content files, no checksum or fixity key, and no vocabulary of descriptive metadata keys. It takes no position on whether a container may be modified after it is written.

## 6. Security considerations

**Identifying a container is a parse of untrusted input.** A reader cannot know whether a file is a container without decompressing the flyleaf and parsing it as TOML, and it must do that before anything about the file has been established. This is not the position of a general ZIP consumer, which chooses what to extract and may decline. Deflate can return a little over a thousand bytes for every byte it is given, before whatever the TOML parser then spends, and a reader invoked automatically — a scanner, an indexer, a shell extension asked for a preview — is invoked on whatever happens to be in the directory.

An implementation MUST bound the decompressed size of the flyleaf, and the depth to which it parses that member, and MUST report a container exceeding either bound as undetermined rather than as non-conformant.
...
The bounds themselves are a matter for the implementation. A number fixed here would be wrong for a reader running on a phone and wrong again for one running over an archive, and the format has no way to know which it is. Undetermined is the verdict because §2.2 already gives that answer for a flyleaf that cannot be read, and a reader answering non-conformant instead would be calling a large but legitimate container malformed on the strength of its own configuration.

**Undetermined can be arranged.** A container whose flyleaf is encrypted is undetermined by §2.2, which follows from the format defining no encryption of its own and forbidding none. The consequence is worth stating plainly: a program treating undetermined as *skip* can be made to skip on purpose, while the content file sits in the same archive unencrypted and legible to anything that never consulted the flyleaf. Undetermined is a reason to look further, not a reason to stop.
...
By convention a container is named for its content file with `.slpc` appended: `foo.pdf.slpc` holds `foo.pdf`. This keeps the content file's own extension visible and survives content files with double extensions or none at all.

It is a convention and nothing more. Nothing enforces it, a container may be named anything, and `content.file` is the only authority on the content file's name.

## Appendix C. Relationship to version 1.0

A container declaring `1.0` is governed by [version 1.0](https://github.com/excelano/slipcase/blob/v1.0/SPEC.md) of this specification, which names the flyleaf `slipcase.metadata.toml` and the content file key `payload.file`. Under this version those names carry no meaning: a member named `slipcase.metadata.toml` is an additional member under §2.1, and a `payload` table is an additional key under §2.2.

A reader establishes which version a container declares by reading its flyleaf, and the flyleaf's name is one of the things that differs. A reader that knows only this version's name finds no flyleaf in a 1.0 container and cannot reach the verdict §2.4 describes; reaching it takes knowing the 1.0 name as well. The name is the one thing in a container a reader has to know before it can learn anything else, and a version that changes it is unreadable, rather than out of scope, to every reader written before it.

Nothing else differs between the two versions. A conformant 1.0 container becomes a conformant 1.1 container by renaming its flyleaf, renaming the key within it, and changing the value of `slipcase_version`, and the reverse holds. §2.4 still applies: an implementation that recognizes one version and not the other reports a container declaring the other as outside its conformance question, not as non-conformant.
