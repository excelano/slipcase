...
A reader establishes which version a container declares by reading its flyleaf, and the flyleaf's name is one of the things that differs. A reader that knows only this version's name finds no flyleaf in a 1.0 container and cannot reach the verdict §2.4 describes; reaching it takes knowing the 1.0 name as well. From this version on the flyleaf's name does not change, so that the version key stays reachable across every later pair of versions.
