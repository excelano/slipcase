...
- `payload.file` MUST be a plain filename: non-empty, not `.` or `..`, and containing no `/`, `\`, or `:`. A payload is one file, not a path into a tree — and a name that cannot express a path cannot express a traversal. The colon is excluded because on some platforms a name containing one is treated as rooted, so joining it to a destination directory discards the destination.
- The payload member MUST NOT be a symbolic link entry. Otherwise a container could hand an application a link to something it was never meant to read.
